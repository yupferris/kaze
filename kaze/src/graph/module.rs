...
///
/// Once a `Module` is specified, it can be [instantiated](#method.instance) in another `Module` to form a hierarchy, or it can be used to generate [Rust simulator code](sim/fn.generate.html) or a [SystemVerilog module](system_verilog/fn.generate.html).
///
